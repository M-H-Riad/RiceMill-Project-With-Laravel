    ...
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      ...
      <div style="margin-left: 260px;margin-top: 20px;">
        ...
        <table class="table table-hover table-bordered">
            ...
            @foreach($othercost as $row)
            <tr>
              <td>{{ $row['updated_at']}}</td>
              <td>{{ $row['costName']}}</td>
              <td>{{ $row['productQuantity']}}</td>
              <td>{{ $row['totalCost']}}</td>
              <td><a href="{{action('otherCostController@edit',$row['id'])}}"><input class="btn btn-warning" type="button" value="Update"></a><hr>
              <form method="post" action="{{ action('otherCostController@destroy',$row['id'])}}">
                    {{ csrf_field() }}
