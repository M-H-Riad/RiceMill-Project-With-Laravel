    ...
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      ...
      <div style="margin-left: 260px;margin-top: 20px;">
        ...
        <table class="table table-hover table-bordered">
            ...
            @foreach($riceprofit as $row)
            <tr>
              <td>{{ $row['updated_at']}}</td>
              <td>{{ $row['riceName']}}</td>
              <td>{{ $row['riceQuality']}}</td>
              <td>{{ $row['riceQuantity']}}</td>
              <td>{{ $row['unitPrice']}}</td>
              <td>{{ $row['totalPrice']}}</td>
              <td>{{ $row['customerName']}}</td>
              <td>{{ $row['customerPhone']}}</td>
              <td>{{ $row['customerAddress']}}</td>
              <td><a href="{{action('riceProfitController@edit',$row['id'])}}"><input class="btn btn-warning" type="button" value="Update"></a><hr>
              <form method="post" action="{{ action('riceProfitController@destroy',$row['id'])}}">
                    {{ csrf_field() }}
