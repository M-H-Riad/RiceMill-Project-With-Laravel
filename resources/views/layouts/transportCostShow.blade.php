      ...
      <div style="margin-left: 260px;margin-top: 20px;">
        ...
        <table class="table table-hover table-bordered">
            <tr>
              ...
              <th class="text-center">Num Of Vehicle</th>
              <th class="text-center">Source</th>
              <th class="text-center">Destination</th>
              <th class="text-center">Cost Per Vehicle</th>
              <th class="text-center">Total Cost</th>
              <th class="text-center">Action</th>
            </tr>

            @foreach($transportcost as $row)
            <tr>
              <td>{{ $row['updated_at']}}</td>
              <td>{{ $row['transportName']}}</td>
              <td>{{ $row['transportNumber']}}</td>
              <td>{{ $row['source']}}</td>
              <td>{{ $row['destination']}}</td>
              <td>{{ $row['transportCost']}}</td>
              <td>{{ $row['totalCost']}}</td>
              <td><a href="{{action('transportCostController@edit',$row['id'])}}"><input class="btn btn-warning" type="button" value="Update"></a> <hr>
              <form method="post" action="{{ action('transportCostController@destroy',$row['id'])}}">
                {{ csrf_field() }}
