      ...
      <div style="margin-left: 260px;margin-top: 20px;">
        ...
        <table class="table table-hover table-bordered">
            <tr>
              ...
              <th class="text-center">Number Of Worker</th>
              <th class="text-center">Hour</th>
              <th class="text-center">Cost Per Worker</th>
              <th class="text-center">Total Cost</th>
              <th class="text-center">Action</th>
            </tr>

            @foreach($workercost as $row)
            <tr>
              <td>{{ $row['updated_at']}}</td>
              <td>{{ $row['numOfWorker']}}</td>
              <td>{{ $row['workingHour']}}</td>
              <td>{{ $row['costPerWorker']}}</td>
              <td>{{ $row['totalCost']}}</td>
              <td><a href="{{action('workerCostController@edit',$row['id'])}}"><input class="btn btn-warning" type="button" value="Update"></a>  <hr>
              <form method="post" action="{{ action('workerCostController@destroy',$row['id'])}}">
                    {{ csrf_field() }}
    ...
    <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalCenterTitle">Delete Information</h5>
     ...
     <div class="modal-body">
      <!-- Modal body -->
        <div>
          <p class="text-danger">Are you sure want to delete this data?</p>
        </div>
     </div>
